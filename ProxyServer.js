...
function finish_websocket(upg_reqhost, proxy_server, ws) {
  ...
  */
  ...
  var headersToSkip = ["sec-websocket-protocol", "cookie", "origin", "upgrade", "connection", "x-client-cert-s-dn", "ezb_user_info_json", "ezb_user_info_json_signature", "pragma", "cache-control", "sec-websocket-key", "sec-websocket-version", "sec-websocket-extensions", "user_agent"]; 
  for (var attrname in upgrade_req.headers) { 
    if (headersToSkip.indexOf(String(attrname)) < 0) {
      zheaders.headers[attrname] = upgrade_req.headers[attrname]; 
    }
  }
